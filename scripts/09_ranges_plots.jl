...
effs_estimations0 = CSV.read(datadir("efficiency_estimations-n_at_pmax0.csv"), DataFrame)
effs_estimations1 = CSV.read(datadir("efficiency_estimations-n_at_pmax1.csv"), DataFrame)

@select!(effs_estimations0, :sim, :variable, :offset, :rmse)
@select!(effs_estimations1, :sim, :variable, :offset, :rmse)

joined = leftjoin(
    effs_estimations0, effs_estimations1; on=[:sim, :variable, :offset], makeunique=true
)
disallowmissing!(joined)
@rtransform!(joined, :comp = :rmse ≈ :rmse_1, :diff = :rmse - :rmse_1)
...
equals = @chain joined begin
    @rsubset :offset < 0.0
    @rsubset :comp
end

comps = @chain joined begin
    @rsubset :offset < 0.0
    @rsubset !:comp
    # @rsubset :diff < 0.0
    # @rsubset :diff > 0.0
    ...
    @rtransform(:prop_pos = :npos / :n, :prop_neg = :nneg / :n)
end

ncomps = sum(comps.n)
prop_pos = sum(comps.npos) / ncomps
prop_neg = sum(comps.nneg) / ncomps

ntot = ncomps + nrow(equals)
prop_pos = sum(comps.npos) / ntot
prop_neg = sum(comps.nneg) / ntot
prop_eq = nrow(equals) / ntot

comps2 = @chain joined begin
    @rsubset :offset < 0.0
    @rsubset !:comp
    # @rsubset :diff < 0.0
    # @rsubset :diff > 0.0
    sort!(:offset)
    @rtransform!(:sign = :diff > 0 ? "pos" : "neg")
    groupby([:offset, :sign])
    @combine(:mean = mean(abs.(:diff)))
    unstack(:sign, :mean)
end
count(>(0), comps2.pos .- comps2.neg) / nrow(comps2)
count(<(0), comps2.pos .- comps2.neg) / nrow(comps2)
# Presque toujours négatif, donc différence positive inférieure à différence négative
